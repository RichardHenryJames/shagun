...
**Current status — 2026-09-08: source pushed, hosted CI passed; framework config fix awaiting redeploy verification; real inventory blocked.** The latest full `npm run check` passed **697 tests across 9 files**, with lint, route type generation and strict typecheck green. The original 692-test set is unchanged and all five new database-access source-contract tests passed. The previously passing normal Next.js production build already includes the action/research changes; later minor changes are scripts, documentation and source-gate checks, with no new normal build for this docs update. Earlier **207 browser passes / 22 skips are historical local isolated QA only**; no new full local matrix is recorded. The default normal preview at http://localhost:3000 is **empty/unconfigured**, not live Hazaribag data. All 12 researched candidates remain unimported. No working public deployment or production certification is claimed.

The shared Supabase project and imported Vercel project already exist. Read-only inspection with verified TLS recognizes **already-configured Supautils delegation to manage policies on `storage.objects`**; the earlier `MIGRATION_PRIVILEGES` denial was a **local-runner false positive**, now resolved. Inspection now gets beyond permissions and fails **`stage=metadata-public code=22023`** in the catalog/preservation snapshot. The earlier SQL syntax error `42601` was fixed and parsed in isolated PGlite, but that does not resolve the managed runtime failure. **No apply, seed, provisioning, research import or database write occurred; `shagun` does not exist. Further database writes/debugging are stopped pending review.** Shagun API keys and a real admin account remain unavailable. Initial root commit [2edd4fd](https://github.com/RichardHenryJames/shagun/commit/2edd4fd831808e6b8ffdf30c46d7265bc734c410) was **successfully pushed**, and its hosted workflow **completed / SUCCESS**. Vercel reached **Ready**, but production still returns **Vercel 404**; the explicit Next.js framework fix awaits parent push/redeploy verification. This documentation-only follow-up remains **uncommitted/unpushed, pending the parent**.
...
- Use Node.js **24** as the local/deployment baseline with the locked TypeScript **5.9.3** dependency. The recorded Windows verification used **Node v24.15.0 / npm 11.12.1**; the current engine range is `>=22.0.0 <25`. The Node 22 hosted workflow passed for initial commit `2edd4fd`, separately from these local results; the pending follow-up is not covered.
...
| --- | --- |
| GitHub repository | **Published:** [RichardHenryJames/shagun](https://github.com/RichardHenryJames/shagun) contains the **161-file root commit [2edd4fd](https://github.com/RichardHenryJames/shagun/commit/2edd4fd)** on `main`; the initial push succeeded. |
| Git authentication | The successful push used Git Credential Manager's existing **`RichardHenryJames`** owner credential. No Git user identity was changed. The separate `gh` CLI account's `push=false` is a different credential context, not a blocker after the confirmed Git push. |
| Local/remote tracking | `origin` targets the repository above and `main` tracks `origin/main`. The initial code is committed and pushed; **this final documentation-only follow-up is not yet committed/pushed** and is not part of the linked code revision. |
| Release safety | The recorded pre-publication release check covered **161 files with no findings**; no secrets were leaked. Review the final documentation diff and secret exclusions before its separate publication; no new scan is claimed for this docs-only update. |
| Hosted CI | [Run 34222982087](https://github.com/RichardHenryJames/shagun/actions/runs/34222982087) **completed / SUCCESS** for `2edd4fd831808e6b8ffdf30c46d7265bc734c410`. Node 22 workflow includes check, QA build and Chromium; no hosted job/test counts fetched. |
| Vercel | Initial deployment `GExD5d9et2YdMJ1p9eWpNJBsWMX4`: **Ready in 43 seconds**, but [https://shagun-peach.vercel.app](https://shagun-peach.vercel.app) still returns **Vercel 404**. Preview URL is SSO/Auth-gated; no bypass attempted. |
| Framework fix | Empty-repository import left **Framework Preset: Other**. Root [../vercel.json](../vercel.json) explicitly sets `framework: nextjs`, `installCommand: npm ci` and `buildCommand: npm run build`. Parent push will trigger redeployment; actual public route verification remains pending. |
...
Do not bootstrap around unavailable sign-ins or fabricate accounts/credentials. Existing delegated policy authority, the confirmed Git push and initial-commit hosted CI success do not establish successful full database inspection, API-key/admin access or working public deployment; those gates remain separate.
...
Use [../scripts/db-migrate.ts](../scripts/db-migrate.ts) with [../scripts/db-migration-plan.ts](../scripts/db-migration-plan.ts), in an approved operator context, **not an application JWT or standalone autocommit paste**. The completed **697-test full check** includes **188 passing migration-plan tests and five passing database-access source-contract tests**; the original 692 tests are unchanged. The isolated Storage fixture's realistic managed-role permissions are **test-only**, and the five source-contract tests do not execute SQL or emulate Supautils. Local tests neither grant managed authority nor certify the provider's delegation hook or snapshot runtime; native permissions and all other guards remain authoritative.
...
1. Retain the confirmed full `npm run check` result: **697 tests across 9 files**, lint and strict typecheck green, including all five new database-access tests. For subsequent code changes and launch acceptance, refresh the appropriate local QA using [../README.md](../README.md): `npm run check`, `npm run build:qa`, then Chromium tests. The recorded 207 passes are historical, not a new run of the latest changes. [../scripts/qa.ts](../scripts/qa.ts), [../scripts/qa-environment.ts](../scripts/qa-environment.ts) and [../playwright.config.ts](../playwright.config.ts) retain the exact **localhost:3100** compile/server origin and labelled isolated fixtures. Stop the normal **Shagun: Local app preview** before replacing shared build output and any optional QA preview before Playwright takes port 3100.
2. Initial code publication and hosted CI are complete for `2edd4fd`; this does not cover the pending follow-up. The imported Vercel project used **Other**, and its initial **Ready** deployment still serves production **404**. The added root [../vercel.json](../vercel.json) selects `nextjs`, `npm ci` and `npm run build`; the parent will push the fix to trigger redeployment. Verify the resulting revision, framework, Node **24** and actual production routes; do not bypass preview SSO or add migrations, seeds, provisioning or smoke mutations to build hooks. Review this documentation-only diff and secret exclusions before its **separate parent commit/push**, still pending.
...
[../.github/workflows/ci.yml](../.github/workflows/ci.yml) is configured to use Node 22 and `npm ci`, run `npm run check`, build through `npm run build:qa`, install Playwright Chromium and run `npm run test:e2e`. [Hosted run 34222982087](https://github.com/RichardHenryJames/shagun/actions/runs/34222982087) **completed / SUCCESS** for `2edd4fd831808e6b8ffdf30c46d7265bc734c410`; no hosted job/test counts were fetched, and this pass does not cover the pending framework/docs follow-up. Service variables are empty, repository permissions are read-only, checkout credentials are not persisted, and browser reports/results are uploaded on failure. There is no deployment or managed-database migration job and no managed-service secret reference.

**Completed full local check (2026-09-08):** Node **v24.15.0 / npm 11.12.1**, TypeScript **5.9.3**, Next.js **16.3.3**. `npm run check` passed lint, route type generation, strict typechecking and **697 tests across 9 files**. The original 692 tests are unchanged and all five new database-access tests passed. [VERIFICATION.md](VERIFICATION.md) lists every count, including 197 database, 188 migration-plan, 121 research and five database-access tests. The **previous normal `npm run build` pass already includes the action/research changes**; later minor changes are scripts, documentation and source-gate checks. No new normal build, full browser matrix, QA build or command execution is claimed by this docs-only refresh.
...
The unchecked items below require their stated **real managed-service or hosted-release evidence**. The full **697-test / 9-file local check, initial code push and initial-commit hosted CI success are confirmed**; this documentation-only follow-up remains uncommitted/unpushed, pending the parent. The older local browser matrix remains historical, with no hosted counts fetched. Vercel's initial **Ready** deployment still serves production **404**; framework-fix redeploy verification and the full inspection, migration, Auth and inventory-publication gates remain open.

### Resources, configuration and build

- [x] **LOCAL:** Full `npm run check` passed lint, route type generation, strict typecheck and **697 tests across 9 files**, including all five new database-access tests. The original 692 tests are unchanged; the previous normal build already includes the action/research changes. This is not hosted or managed-service certification.
- [x] **INITIAL SOURCE PUSH:** Root commit [2edd4fd](https://github.com/RichardHenryJames/shagun/commit/2edd4fd), **161 files**, successfully pushed to `main` through Git Credential Manager's existing `RichardHenryJames` owner credential, with `origin/main` upstream. No user identity change; release check covered 161 files with no findings and no secrets leaked. This does not include the pending final documentation follow-up.
- [x] **INITIAL HOSTED CI:** [Run 34222982087](https://github.com/RichardHenryJames/shagun/actions/runs/34222982087) **completed / SUCCESS** for `2edd4fd831808e6b8ffdf30c46d7265bc734c410`, with the Node 22 check/QA-build/Chromium workflow. No hosted test counts fetched; no follow-up revision or public deployment certification implied.
- [ ] **BLOCKED:** Authorized Supabase application configuration, API/service keys supplied privately and a real admin account/session remain unavailable. Existing delegated Storage-policy authority is recognized; no additional owner authority is required by the diagnosis. Confirm resource ownership, region, current commercial terms and budget; initial Vercel Ready does not resolve the observed production 404.
...
- [ ] **PENDING:** Parent publication of the framework fix and this uncommitted documentation-only follow-up; check hosted CI for the resulting revision and verify Vercel redeployment plus actual production routes, not Ready alone. Confirm Next.js, Node 24, fixtures disabled and correct scoped configuration. Initial-commit CI success is already recorded; no working public deployment is yet verified.

...
- Historical Preview/non-Production deployments, the original catalog smoke defect and earlier branch observations are retained in [VERIFICATION.md](VERIFICATION.md). They are not fresh branch/process checks or proof of a global Vercel production-branch setting. Review Preview access requires Vercel authorization, not a Shagun customer login.

...
**As of 2026-09-09: SQL installation and Production secret storage are complete; production promotion is blocked.** Guarded `--apply --seed` succeeded on shared Supabase **`ixkhyqqovacdramymqjk`**. Critical anonymous Order/ContactMessage access is now confirmed; Shagun is missing from the API schema list, and there is no approved real administrator or reviewed inventory. **The next push is to a review branch only, not production. No force launch or promotion is authorized.**

This is an operator runbook, not authorization to deploy or mutate shared settings. No command, migration, provisioning, Git operation, new service probe or deployment was run for this documentation update. Work/evidence spans 8–9 September; counts, preservation flags, limited restore evidence and pending results are in [VERIFICATION.md](VERIFICATION.md).
...
- The planned review branch is **`audit/second-pass-2026-09-09`**, not yet pushed. Confirm the push cannot trigger production promotion, then record its actual revision and hosted checks. Do not merge to the production branch, redeploy Production or enable live inventory to bypass the security hold.
...
The integration build and **3/3 real local Supabase workflows passed**; the latter completed in **2.4 minutes after the signed-in axe/no-overflow checks and `.a-table-wrap` positioning fix**. That Auth/Chapra/venue/media/publishing/cleanup run **preceded the sitemap refactor** and was not rerun afterward; post-refactor SEO coverage is unit/public tests. The final normal build and full public matrix also passed. Preserve these completed labels in [VERIFICATION.md](VERIFICATION.md); only `FINAL_RELEASE_RESULT` remains pending among the final-result labels:

- [x] **`FINAL_CHECK_RESULT`:** `npm run check` passed lint/typechecks and **950 tests across 14 files**, including **19 request-time sitemap tests** and the existing integration-reset guards.
- [x] **`FINAL_NORMAL_BUILD_RESULT`:** final normal `npm run build` **passed with `SHAGUN_TEST_FIXTURES=false`**, dynamic request-time sitemaps and **no database queries during compilation**. Never deploy fixture/integration output or share generated output with a running preview during rebuild.
- [x] **`FINAL_FIXTURE_MATRIX_RESULT`:** fresh `many` **166 passed / 1 skip**, `empty` **95 passed / 28 skips**, and `one` **120 passed / 21 skips**, each at **320, 375, 390, 414, 768 and 1440 px**. **Total: 381 passed / 50 intentional, inapplicable skips**, no failures or weakened assertions.
- [ ] **`FINAL_RELEASE_RESULT`:** planned branch **`audit/second-pass-2026-09-09` is not yet pushed**. Record the actual push/revision and hosted CI result only once confirmed; do not add an unconfirmed push URL. The new two-job workflow runs Node **22** quality/fixture checks, then Node **24** real local Auth/Storage tests with Docker; no credential-bearing output/artifacts are emitted, and the integration stop step uses `always()`. Hosted execution is pending. **No production promotion.**
...
The normal build is complete; default port-3000 preview restoration is being checked, with current availability not yet confirmed. An empty connected directory is correct before publication. No real Hazaribag launch or production promotion is authorized while the shared-security, API, admin and editorial blockers remain. Neither deployment nor research import auto-publishes inventory. Follow [OPERATIONS.md](OPERATIONS.md).

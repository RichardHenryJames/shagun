...
City and global venue tables include page-scoped draft/unpublished selection and **Review & publish**. The native modal reuses existing focus containment, Escape and opener restoration. It displays saved facts/private sources, per-row blockers, optional exclusion and one explicit editorial confirmation before publishing at most 25 selected versions. Confirmation resets on inclusion changes and reopening; published/archived rows stay disabled. Results remain visible after the list refresh, distinguish uncertain saves from failures, and warn when the parent city is not active. The dialog fits 320 px layouts with its own vertical scrolling; no private source notes enter shared public views.

...
**As of 2026-09-09:** the five migrations and draft Hazaribag seed are installed; there are no venues, photos or admins. Production keys are saved in Vercel. **Production promotion is blocked:** shared Order/ContactMessage exposure is confirmed, Shagun's API schema is missing (406 / PGRST106), dashboard access needs private sign-in after a 401, and no approved real admin email is available. All **12 research candidates remain unimported/unreviewed**, without photos or established rights. The [review branch](https://github.com/RichardHenryJames/shagun/tree/audit/second-pass-2026-09-09) is published through `9db51bd`; original `2072dc7` hosted audit checks passed. Follow-up code status is canonical in [VERIFICATION.md](VERIFICATION.md). No `main`/Production promotion or force launch; follow the owner/operator gates in [DEPLOYMENT.md](DEPLOYMENT.md).

## Local walkthrough versus real administration

The default normal preview is localhost:3000 with fixtures disabled and normally no database. After the catalog guard, its normal fixtures-false build passed and the restored browser preview confirmed the honest empty preparation state without synthetic warnings or inventory. Labelled public fixtures use 3100; real local Supabase integration uses Next on 3200 and API/database ports 55321/55322. The latter tests actual services with temporary synthetic inventory, not the managed project. Commands and build/process ownership rules are in [../README.md](../README.md). Never create manual production test records; no Chapra production record has been created.

Historical Windows **3/3** workflows at 390, 768 and 1440 px passed after the table-wrapper/axe/no-overflow fix and **before the sitemap refactor**. Separate hosted Node 24 Auth/Chapra/venue/media/publishing/cleanup workflows passed **after the sitemap refactor at `2072dc7`**. These isolated-service runs do not certify the later catalog guard or managed production acceptance; exact scopes are in [VERIFICATION.md](VERIFICATION.md).
...
[VERIFICATION.md](VERIFICATION.md) records the post-fix **952-test / 14-file** local check, normal fixtures-false build and actual valid/malformed catalog 503 responses. It preserves the earlier Windows **381 passes / 50 intentional skips** across all three scenarios and six widths, separately from original hosted `2072dc7` CI's `many`-only fixtures and post-sitemap Auth pass. Follow-up status and observed Preview/non-Production deployment with unchanged remote/local `main` belong there. [DEPLOYMENT.md](DEPLOYMENT.md) remains the operator checklist; this document authorizes no production promotion, automatic provisioning/publication or sibling changes.
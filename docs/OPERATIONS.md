...
The batch is bounded and **not a single transaction**. A provider failure or deadline can leave confirmed drafts plus an unconfirmed row and rows not attempted. Inspect the saved inventory before revalidating/retrying an interrupted import; do not assume an unconfirmed request failed to commit. Keep the same slugs to preserve retry matching. Split larger work into separate batches of at most 100 rows; this is not a background bulk job. The application processes the workbook in memory and does not retain the uploaded file in Storage.

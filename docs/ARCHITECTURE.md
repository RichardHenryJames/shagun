...
Bulk publication uses `bulkPublishVenuesAction` in [../src/lib/actions/venues.ts](../src/lib/actions/venues.ts), not a new migration or direct table update. Both review and publish modes freshly authorize and accept at most **25 unique ID/exact-version targets**. Review returns bounded private saved facts/source notes without writes. Publish requires explicit boolean review approval, rereads every selected record, rejects changed versions or incomplete details, and builds the sole `save_venue` payload from saved data rather than posted facts. Status becomes published and SQL supplies review attribution; facts, sources, facilities, verification/date and city state remain unchanged. Existing RLS/SQL grants and constraints stay authoritative. A 35-second operation deadline, 5-second reads, 8-second saves and durable per-admin rate limit bound work. Results preserve confirmed, blocked, skipped, conflicted, unattempted and uncertain outcomes; an uncertain save stops continuation without an automatic retry. Inventory is invalidated after confirmed or possible mutations.

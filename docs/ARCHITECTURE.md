...
**Current status — 2026-09-08: source pushed, hosted CI passed; framework config fix awaiting redeploy verification; real inventory blocked.** The latest full `npm run check` passed **697 tests across 9 files**, lint, route type generation and strict typecheck. The original 692 tests are unchanged and all five new database-access source-contract tests passed. The previous normal Next.js production build already includes the action/research changes; later minor changes are scripts, documentation and source-gate checks, with no new normal build for this docs update. Initial commit [2edd4fd](https://github.com/RichardHenryJames/shagun/commit/2edd4fd831808e6b8ffdf30c46d7265bc734c410) is pushed to `main` and its hosted workflow **completed / SUCCESS**. Vercel's initial deployment reached **Ready**, but production still returns **Vercel 404**. The default port-3000 app is **empty/unconfigured**, not live inventory. All 12 researched candidates remain unimported. Read-only Supabase inspection recognizes existing delegated Storage-policy authority but now fails **`stage=metadata-public code=22023`** in the catalog/preservation snapshot, not at a permissions gate. Database work is stopped pending review; API keys/admin access remain unavailable. Earlier **207/22 browser results are historical local QA only**, with no new full local matrix. See [VERIFICATION.md](VERIFICATION.md) and [DEPLOYMENT.md](DEPLOYMENT.md).
...
- Stable Next.js App Router + React + strict TypeScript 5.9.3. Node.js **24** is the local/deployment baseline; the supported engine range is `>=22.0.0 <25`. Local verification used Node **v24.15.0** / npm **11.12.1**. The Node 22 workflow (check, QA build and Chromium) [completed / SUCCESS for initial commit 2edd4fd](https://github.com/RichardHenryJames/shagun/actions/runs/34222982087); no hosted test counts were fetched, and the pending follow-up is not covered.
...
Vercel Node runtime + the selected shared Supabase project. No container or always-on app server. The **161-file root commit [2edd4fd](https://github.com/RichardHenryJames/shagun/commit/2edd4fd831808e6b8ffdf30c46d7265bc734c410)** was successfully pushed to `main` in [RichardHenryJames/shagun](https://github.com/RichardHenryJames/shagun), with `main` tracking `origin/main`, and its hosted CI passed. Git Credential Manager used the existing `RichardHenryJames` owner credential; no Git user identity was changed. The recorded release check covered 161 files with no findings and no secrets leaked. The separate `gh` CLI account's `push=false` does not negate this confirmed Git push. The empty-repository import left Vercel's preset at **Other**; the initial deployment reached **Ready in 43 seconds**, but [production](https://shagun-peach.vercel.app) still returns **Vercel 404**. Root [../vercel.json](../vercel.json) now sets `framework: nextjs`, `installCommand: npm ci` and `buildCommand: npm run build`; parent push and redeploy verification are pending. This documentation-only follow-up remains **uncommitted/unpushed, pending the parent**. No working public deployment is verified.

Supabase API keys and a real admin account remain unavailable. The snapshot code blocker must be reviewed/fixed, and all **four ordered SQL migrations** must pass full read-only inspection and be explicitly authorized for atomic application before the optional draft-city seed and a connected inventory launch; builds NEVER seed or reset a database. Keep staging separate from the shared live project, and do not connect preview deployments to production data for editing. A repository build of the unconfigured preparation state is not managed-service or inventory-launch acceptance.
...
6. Database credentials and delegated policy capability are not API keys or a real admin session; those remain unavailable. Initial code push and its hosted CI succeeded; final documentation publication and framework-fix redeploy verification remain pending. Vercel Ready did not establish working production routes. Neither local nor hosted tests certify managed services, complete city coverage or current contacts.
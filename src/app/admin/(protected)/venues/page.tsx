...
export default async function AdminVenuesPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  return (
    <>
      ...
      {result.items.length ? <VenueTable venues={result.items} /> : (
      ...
      <p className="a-field-hint">Change publication status in a venue’s Publishing section and save. Preview links are authenticated and show only saved content.</p>
    </>
  );
}
...
import { VENUE_IMPORT_MAX_BYTES, type VenueImportReport } from "@/lib/venue-import";
...
export async function POST(request: Request) {
  try {
    ...
    if (!isConfigured()) throw new HttpError(503, "Secure server setup is incomplete.");
    const { client, admin } = await freshAdminContext();
    await enforceRateLimit("admin-venue-workbook", admin.id, 12, 60);
    const form = await readBoundedFormData(request, VENUE_IMPORT_MAX_BYTES + 16 * 1024);
    const allowed = new Set(["file", "city_id", "mode", "digest"]);
    ...
    const { data: existing, error: lookupError } = await client.from("venues").select("id,slug")
      .eq("city_id", city.id).in("slug", parsed.rows.map(({ input }) => input.slug)).abortSignal(AbortSignal.timeout(5000));
    if (lookupError || !existing || existing.some((venue) => !uuidSchema.safeParse(venue.id).success)) throw new HttpError(503, "Existing venues could not be checked. No drafts were imported.");
    const existingBySlug = new Map(existing.map((venue) => [venue.slug, venue.id]));
    report.rows = parsed.rows.map(({ row, input }) => {
    ...
    report.remaining = report.totalRows - report.skipped;
    if (mode === "validate") return Response.json(report, { headers: PRIVATE_HEADERS });
    await enforceRateLimit("admin-venue-import", admin.id, 3, 60);
    const deadline = Date.now() + 35_000;
    let unconfirmedWrite = false;
    try {
      for (const [index, entry] of parsed.rows.entries()) {
    ...
    report.remaining = report.totalRows - report.created - report.skipped;
    if (report.created > 0 || unconfirmedWrite) {
      try { invalidateInventory(); }
      catch { report.phase = "stopped"; report.error = "Saved drafts could not be reflected in the workspace. Reload and check inventory before retrying."; }
    }
